...
public class Part2 {
    ...
    public static void main(String[] args) {
        ...
        answer = explore(paths, START, visited, false);

        System.out.println("Answer:" + answer);
    }

    static int explore(Map<String, List<String>> paths, String cave, Stack<String> visited, boolean twice) {
        if (cave.equals(END)) {
            return 1;
        }
        if (cave.equals(cave.toLowerCase(Locale.ROOT)) && (visited.search(cave) != -1 && (twice || cave.equals(START) || cave.equals(END)))) {
            return 0;
        }
        ...
        for (String c : paths.get(cave)) {
            i += explore(paths, c, visited, twice);
        }
        visited.pop();
